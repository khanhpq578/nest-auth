...
export class GCloudStorageService {
  ...
  async upload(
    ...
    const filename = uuid();
    const gcFilename =
      perRequestOptions && perRequestOptions.prefix ? perRequestOptions.prefix + '/' + filename : filename;
    // console.log(gcFilename);
  ...
  getStorageUrl(filename: string, perRequestOptions: Partial<GCloudStoragePerRequestOptions> = null) {
    ...
    return 'https://storage.googleapis.com/' + perRequestOptions.defaultBucketname + '/' + filename;
  }
}
